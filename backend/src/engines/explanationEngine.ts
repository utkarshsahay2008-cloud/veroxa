import { CompleteAnalysisResponse, UserProfile, UnclaimedOpportunity } from '../types';
...
export function runFullTaxAnalysis(profile: UserProfile): CompleteAnalysisResponse {
  ...
  const unclaimedOpportunities: UnclaimedOpportunity[] = [];
  ...
  if (totalPotentialExtraSaving > 0) {
    summaryStatements.push(
      `Actionable Tax Saving Opportunity: You have ${unclaimedOpportunities.length} unclaimed tax headroom opportunities that could save you up to an additional ₹${totalPotentialExtraSaving.toLocaleString('en-IN')} in tax.`
    );
  }
