import { UserProfile, TaxAnalysis, RegimeResult, SlabBreakdown } from '../types';
import { loadTaxRules } from '../utils/configLoader';

export function calculateTaxSlabs(taxableIncome: number, slabs: Array<{ min: number; max: number | null; rate: number; description: string }>): { baseTax: number; breakdown: SlabBreakdown[] } {
...
export function computeTaxAnalysis(profile: UserProfile, totalChapter6ADeductions: number = 0): TaxAnalysis {
  const config = loadTaxRules();
  ...
  // --- REGIME COMPARISON ---
  let recommendedRegime: 'Old Tax Regime' | 'New Tax Regime (Default)';
